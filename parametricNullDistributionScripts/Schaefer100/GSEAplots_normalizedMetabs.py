...
for i in range(1,160):
        parallelized_output = pd.read_csv('Schaefer100n40/Bic_ESwiseNull_Lake_' + str(i).zfill(4) + '.csv')
        if i == 1:
...
brain_phenotype_file = "formatted_segstatsFilesSchaefer100/formatted_bstemNormBicSchaefer100.txt"
...
correctedPvalues = []
#print(empiricalPrerank.res2d.columns)
...
ES_nullDistribution_temp = np.array([])
# MISSING LAST 40 NODE FILES, 120 * 6250 permutations, not 160 * 6250 perms
for i in range(1,120):
        parallelized_output = pd.read_csv('Schaefer100n40/Lac_ESwiseNull_Lake_' + str(i).zfill(4) + '.csv')
        if i == 1:
            ES_nullDistribution_temp = np.array(parallelized_output.values) #Because need to have defined size for concatenate
        else:
            ES_nullDistribution_temp = np.concatenate((ES_nullDistribution_temp,np.array(parallelized_output.values)))
...
lake_gmt = "geneset_LAKE.gmt"
geneExpressionCSV = "Schaefer100_lh_expression.csv"
brain_phenotype_file = "formatted_segstatsFilesSchaefer100/formatted_bstemNormLacSchaefer100.txt"
...
for i in range(1,80):
        parallelized_output = pd.read_csv('Schaefer100n40/Pyr_ESwiseNull_Lake_' + str(i).zfill(4) + '.csv')
        if i == 1:
            ES_nullDistribution_temp = np.array(parallelized_output.values) #Because need to have defined size for concatenate
        else:
            ES_nullDistribution_temp = np.concatenate((ES_nullDistribution_temp,np.array(parallelized_output.values)))
...
lake_gmt = "geneset_LAKE.gmt"
geneExpressionCSV = "Schaefer100_lh_expression.csv"
brain_phenotype_file = "formatted_segstatsFilesSchaefer100/formatted_bstemNormPyrSchaefer100.txt"
