...
for i in range(1,384):
        parallelized_output = pd.read_csv('Schaefer200n40/BP_ESwiseNull_Lake_' + str(i).zfill(4) + '.csv')
        if i == 1:
...
for i in range(1,384):
        parallelized_output = pd.read_csv('Schaefer200n40/LP_ESwiseNull_Lake_' + str(i).zfill(4) + '.csv')
        if i == 1:
            ES_nullDistribution_temp = np.array(parallelized_output.values) #Because need to have defined size for concatenate
        else:
            ES_nullDistribution_temp = np.concatenate((ES_nullDistribution_temp,np.array(parallelized_output.values)))
...
EP = empiricalPrerank.plot(terms = significantGenesets,
                           figsize = (3,4))
EP.savefig('OneMill_Schaefer200-LP_SA_0.05.pdf',format ='pdf',bbox_inches="tight")
...
ES_nullDistribution_temp = np.array([])
for i in range(1,384):
        parallelized_output = pd.read_csv('Schaefer200n40/LB_ESwiseNull_Lake_' + str(i).zfill(4) + '.csv')
        if i == 1:
            ES_nullDistribution_temp = np.array(parallelized_output.values) #Because need to have defined size for concatenate
        else:
            ES_nullDistribution_temp = np.concatenate((ES_nullDistribution_temp,np.array(parallelized_output.values)))
...
EP = empiricalPrerank.plot(terms = significantGenesets,
                           figsize = (3,4))
EP.savefig('OneMill_Schaefer200-LB_SA_0.05.pdf',format ='pdf',bbox_inches="tight")
