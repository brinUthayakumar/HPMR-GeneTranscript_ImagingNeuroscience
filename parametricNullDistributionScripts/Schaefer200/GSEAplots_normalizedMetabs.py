...
for i in range(1,160):
        parallelized_output = pd.read_csv('Schaefer200n40/Bic_ESwiseNull_Lake_' + str(i).zfill(4) + '.csv')
        if i == 1:
...
for i in range(1,160):
        parallelized_output = pd.read_csv('Schaefer200n40/Lac_ESwiseNull_Lake_' + str(i).zfill(4) + '.csv')
        if i == 1:
            ES_nullDistribution_temp = np.array(parallelized_output.values) #Because need to have defined size for concatenate
        else:
            ES_nullDistribution_temp = np.concatenate((ES_nullDistribution_temp,np.array(parallelized_output.values)))
...
for i in range(1,160):
        parallelized_output = pd.read_csv('Schaefer200n40/Pyr_ESwiseNull_Lake_' + str(i).zfill(4) + '.csv')
        if i == 1:
            ES_nullDistribution_temp = np.array(parallelized_output.values) #Because need to have defined size for concatenate
        else:
            ES_nullDistribution_temp = np.concatenate((ES_nullDistribution_temp,np.array(parallelized_output.values)))
